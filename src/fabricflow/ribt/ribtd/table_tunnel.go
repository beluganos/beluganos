...
func (t *TunnelTable) WriteTo(w io.Writer) (sum int64, err error) {
	...
	for remote, tun := range t.remotes {
		n, err = fmt.Fprintf(w, "Reote: %s\n", remote)
		sum += int64(n)
		if err != nil {
			return
		}
	...
	for ifname, tun := range t.ifnames {
		n, err = fmt.Fprintf(w, "Ifname: %s\n", ifname)
		sum += int64(n)
		if err != nil {
			return
		}
	...
	for prefix, tun := range t.prefixes {
		n, err = fmt.Fprintf(w, "Prefix: %s\n", prefix)
		sum += int64(n)
		if err != nil {
			return
		}
