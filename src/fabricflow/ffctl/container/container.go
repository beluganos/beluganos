...
func (c *containerCommand) setIfaceFlags(cmd *cobra.Command) *cobra.Command {
	cmd.Flags().StringArrayVarP(&c.excludeDevices, "exclude", "", []string{"eth0", "ens3"}, "Exclude devices.")
	cmd.Flags().StringVarP(&c.vswCmd, "vsw-command", "", containerGovswCmd, "vsw command.")
	return c.setFlags(cmd)
}

func (c *containerCommand) modGovswdPort(name, cmd string, force bool) error {
	log.Debugf("modPort: name:%s cmd:%s", name, cmd)

	ifnames, err := fflib.LXDContainerHostIfnames(name, c.excludeDevices)
	if err != nil {
		return err
	}

	for _, ifname := range ifnames {
		if err := fflib.ExecAndOutput(os.Stdout, c.vswCmd, "interface", cmd, ifname); err != nil {
			if force {
...
func (c *containerCommand) registerPort(name string) error {
	...
	if err := c.modGovswdPort(name, "add", false); err != nil {
...
func (c *containerCommand) unregisterPort(name string) error {
	...
	if err := c.modGovswdPort(name, "delete", true); err != nil {
...
func (c *containerCommand) showInfo(name string) {
	fflib.ExecAndOutput(os.Stdout, "lxc", "info", name)
	fflib.ExecAndOutput(os.Stdout, "lxc", "config", "show", name)
}

func (c *containerCommand) showList() error {
	return fflib.ExecAndOutput(os.Stdout, "lxc", "list")
}

func (c *containerCommand) start(name string) error {
	if err := fflib.ExecAndOutput(os.Stdout, "lxc", "start", name); err != nil {
		return err
	}
...
func (c *containerCommand) stop(name string) error {
	...
	return fflib.ExecAndOutput(os.Stdout, "lxc", "stop", name)
}

func (c *containerCommand) console(name string) error {
	return fflib.ExecAndWait("lxc", "exec", name, "bash")
}
