...
func (c *DhcpCmd) doGetOption(client *client4.Client, option *IPv4Option, ifnames []string) error {
	msg, err := IPv4Exchanges(client, option.OptionCode(), ifnames)
	if err != nil {
		log.Debugf("%s", err)
		return err
	}
...
func (c *DhcpCmd) getOption() error {
	...
	if c.OptCode != 0 {
		option, err = GetIPv4OptionByCode(c.OptCode)
	} else {
		option, err = GetIPv4Option(c.OptName)
	}
	if err != nil {
		log.Debugf("%s", err)
		return err
	...
	ifnames := IfNameList(c.IfPatterns, c.IfNames, c.IfBlacklist)
...
func (c *DhcpToolsCmd) showOptions() error {
	ListIPv4Options(func(name string, option *IPv4Option) {
		fmt.Printf("%s (%d)\n", name, option.Code)
	})
	return nil
}
